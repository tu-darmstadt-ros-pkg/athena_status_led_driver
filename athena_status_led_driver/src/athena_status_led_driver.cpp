...
void AthenaStatusLedDriver::setup()
{
  ...
  // Create controller
  controller_ = std::make_unique<LedRingController>(
    static_cast<size_t>(led_count_), transport_);
  ...
  power_supply_effect_ = std::make_shared<PowerSupplyEffect>(
    static_cast<size_t>(led_count_));
  spot_light_effect_ = std::make_shared<SpotLightEffect>(
    static_cast<size_t>(led_count_));
  rainbow_loading_effect_ = std::make_shared<RainbowLoadingEffect>(
    static_cast<size_t>(led_count_));
  ...
  set_spot_light_srv_ = create_service<athena_status_led_driver_msgs::srv::SetSpotLight>(
    "~/set_spot_light",
    std::bind(&AthenaStatusLedDriver::onSetSpotLight, this,
  ...
  std::shared_ptr<athena_status_led_driver_msgs::srv::SetSpotLight::Response> response)
{
  ...
  spot_light_effect_->setSpotLight(
    request->enable, request->brightness, request->direction_deg, request->width_deg);
}
