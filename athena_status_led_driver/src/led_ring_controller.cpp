...
float LedRingController::brightness() const
{
  return brightness_;
}
...
void LedRingController::tick(double dt)
{
  ...
  if (brightness_ < 1.0f)
  {
    for (auto& pixel : pixels_)
      pixel = pixel.scaled(brightness_);
  }
...
const std::vector<Color>& LedRingController::pixels() const
{
  return pixels_;
}

std::shared_ptr<LedTransport> LedRingController::transport() const
{
  return transport_;
}
