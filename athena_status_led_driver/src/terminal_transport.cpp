...
bool TerminalTransport::send(const std::vector<Color>& leds)
{
  ...
  if (ring_radius_y < 1) ring_radius_y = 1;
  if (ring_radius_x < 2) ring_radius_x = 2;
  ...
  for (size_t i = 0; i < led_count; ++i)
  {
    ...
    double angle = 2.0 * M_PI * static_cast<double>(i) / static_cast<double>(led_count) - M_PI / 2.0;
  ...
  for (int y = 0; y < grid_height; ++y)
  {
    for (int x = 0; x < grid_width; ++x)
    {
      ...
      {
        std::printf("\033[38;2;%d;%d;%dm●",
                    grid[y][x].r, grid[y][x].g, grid[y][x].b);
      }
      else
      {
        std::printf(" ");
